-- ============================================================
-- TALOSPRIMES — Insertion des WorkflowLinks pour la CONFORMITÉ
-- À exécuter dans Supabase SQL Editor
-- ============================================================
-- Prérequis :
--   1. Les workflows n8n conformité doivent être importés
--   2. Au moins 1 tenant doit exister
--   3. La migration 00-MIGRATION-CONFORMITE.sql doit être exécutée
-- ============================================================

-- -------------------------------------------------------
-- ÉTAPE 1 : Créer le module métier "conformite"
-- -------------------------------------------------------
...
BEGIN
  ...
  IF mod_id IS NULL THEN
    RAISE EXCEPTION 'Module métier "conformite" introuvable. Vérifiez l''étape 1.';
  END IF;

  RAISE NOTICE 'Module conformite trouvé: %', mod_id;
  ...
  RAISE NOTICE '✅ 31 workflow_links conformité insérés pour tous les tenants.';
END $$;
...
WHERE type_evenement LIKE 'compta_%'
  AND type_evenement NOT IN (
    'compta_init', 'compta_plan_comptable_list', 'compta_ecriture_create',
    'compta_ecritures_list', 'compta_ecriture_get', 'compta_auto_facture',
    'compta_auto_avoir', 'compta_auto_paiement', 'compta_grand_livre',
    'compta_balance', 'compta_bilan', 'compta_compte_resultat',
    'compta_tva', 'compta_lettrage', 'compta_cloture',
    'compta_dashboard', 'compta_ia_agent'
  )
