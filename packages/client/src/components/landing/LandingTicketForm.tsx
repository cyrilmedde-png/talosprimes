...
const STATUT_LABELS: Record<string, { label: string; color: string; icon: typeof Clock }> = {
  ouvert: { label: 'Ouvert', color: 'bg-blue-500/20 text-blue-300', icon: Clock },
  en_cours: { label: 'En cours', color: 'bg-yellow-500/20 text-yellow-300', icon: Loader },
  en_attente: { label: 'En attente', color: 'bg-orange-500/20 text-orange-300', icon: AlertTriangle },
  resolu: { label: 'Résolu', color: 'bg-green-500/20 text-green-300', icon: CheckCircle },
  ferme: { label: 'Fermé', color: 'bg-slate-500/20 text-slate-400', icon: AlertCircle },
};
...
const selectClass = "px-4 py-2.5 border border-slate-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500/50 bg-slate-900/80";
    ...
    <div className={className}>
      ...
      <div className="flex rounded-lg border border-slate-700 overflow-hidden mb-5">
        <button
          ...
          className={`flex-1 px-4 py-2.5 text-sm font-medium transition flex items-center justify-center gap-2 ${
            mode === 'create' ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 px-4 py-2.5 text-sm font-medium transition flex items-center justify-center gap-2 ${
            mode === 'track' ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'
          }`}
        >
      ...
      {mode === 'create' && status !== 'success' && (
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button type="submit" disabled={status === 'sending'}
            className="w-full px-4 py-3 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-500 transition disabled:opacity-50 shadow-sm flex items-center justify-center gap-2">
            {status === 'sending' ? (
      ...
      {mode === 'create' && status === 'success' && (
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">Ticket créé avec succès !</h3>
          <div className="inline-flex items-center gap-2 bg-slate-800 text-white font-mono text-lg font-bold px-4 py-2 rounded-lg mb-3 border border-slate-700">
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => setStatus('idle')}
              className="px-4 py-2 text-sm border border-slate-700 rounded-lg hover:bg-slate-800 transition text-slate-300">
              Nouveau ticket
            </button>
        ...
        <div className="space-y-4">
          <form onSubmit={handleTrack} className="flex gap-2">
            ...
            <button type="submit" disabled={trackStatus === 'loading'}
              className="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition text-sm font-medium disabled:opacity-50">
              {trackStatus === 'loading' ? <Loader size={16} className="animate-spin" /> : <Search size={16} />}
            </button>
          </form>

          {trackStatus === 'not_found' && (
            <div className="flex items-center gap-2 text-orange-400 text-sm bg-orange-500/10 border border-orange-500/20 p-3 rounded-lg">
            ...
            <div className="border border-slate-700 rounded-xl overflow-hidden">
              ...
              <div className="bg-slate-800/50 p-4 border-b border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-mono font-bold text-white">{trackData.numero}</span>
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${STATUT_LABELS[trackData.statut]?.color || 'bg-slate-500/20 text-slate-400'}`}>
                    {STATUT_LABELS[trackData.statut]?.label || trackData.statut}
                  </span>
                </div>
                <h4 className="font-semibold text-white text-sm">{trackData.sujet}</h4>
                  ...
                  {trackData.replies.map((r, i) => (
                    <div key={i} className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/50">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-white">{r.auteur}</span>
