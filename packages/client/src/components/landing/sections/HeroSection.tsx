...
export function HeroSection({ config }: { config: HeroConfig; theme?: ThemeConfig }) {
  ...
  return (
    <section className="relative pt-32 pb-8 px-6 overflow-hidden min-h-[90vh] flex flex-col justify-center">
      {/* Animated gradient background — even darker */}
      <div className="absolute inset-0 bg-gradient-to-b from-slate-200 via-slate-100 to-slate-200/90 pointer-events-none" />

      {/* Animated orbs — richer colors */}
      <div className="absolute top-20 left-1/4 w-[600px] h-[600px] bg-gradient-to-br from-blue-300/40 via-violet-200/30 to-transparent rounded-full blur-3xl pointer-events-none animate-orb-1" />
      <div className="absolute top-40 right-1/4 w-[500px] h-[500px] bg-gradient-to-bl from-amber-300/35 via-rose-200/25 to-transparent rounded-full blur-3xl pointer-events-none animate-orb-2" />
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-gradient-to-t from-blue-200/50 to-transparent rounded-full blur-3xl pointer-events-none" />

      {/* Animated grid pattern overlay — shimmer + drift */}
      <div className="absolute inset-0 pointer-events-none animate-grid-shimmer"
        style={{
